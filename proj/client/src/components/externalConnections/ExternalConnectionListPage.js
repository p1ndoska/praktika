...
const ExternalConnectionListPage = () => {
  ...
  const fetchConnections = async (page = 1) => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:5000/api/external-connections?page=${page}&limit=${limit}`, {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/external-connections/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
